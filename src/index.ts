...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Cms Open Payments');
}
...
/** The aggregate mirror. Injected by the gateway; absent in local runs. */
function totalsMirror(args: Record<string, unknown>) {
  const url = (args._supabaseUrl as string | undefined)?.trim();
  const key = (args._supabaseKey as string | undefined)?.trim();
  return url && key ? { url, key } : null;
}

async function topCompanies(args: Record<string, unknown>) {
  const m = totalsMirror(args);
  if (!m) {
    return {
      found: false,
      reason: 'mirror_unavailable',
      message: 'The Open Payments company summary is not reachable from this deployment. The live CMS API cannot answer a ranking question — it has no aggregate endpoint and grouping over the payment table does not return.',
    };
  }
  const limit = Math.min(200, Math.max(1, Number(args.limit) || 25));
  const company = typeof args.company === 'string' ? args.company.trim() : '';

  // Default to the newest year LOADED, not the current calendar year: CMS
  // publishes a program year months after it ends, so guessing "this year"
  // would return nothing and read as "nobody paid anything".
  let year = Number(args.program_year) || 0;
  if (!year) {
    const res = await pwFetch(
      `${m.url}/rest/v1/open_payments_company_totals?select=program_year&order=program_year.desc&limit=1`,
      { headers: { apikey: m.key, Authorization: `Bearer ${m.key}` } },
    );
    if (!res.ok) throw new Error(`Open Payments summary lookup failed (HTTP ${res.status})`);
    const rows = (await res.json()) as { program_year: number }[];
    year = rows[0]?.program_year ?? 0;
    if (!year) {
      return { found: false, reason: 'summary_not_loaded', message: 'No program year has been loaded into the Open Payments company summary yet.' };
    }
  }

  const filter = company
    ? `&company=ilike.*${encodeURIComponent(company)}*`
    : '';
  const res = await pwFetch(
    `${m.url}/rest/v1/open_payments_company_totals?program_year=eq.${year}${filter}` +
    `&select=company,company_id,total_usd,payment_count,distinct_recipients,source_url,refreshed_at` +
    `&order=total_usd.desc&limit=${limit}`,
    { headers: { apikey: m.key, Authorization: `Bearer ${m.key}` } },
  );
  if (!res.ok) throw new Error(`Open Payments summary query failed (HTTP ${res.status})`);
  const rows = (await res.json()) as Record<string, unknown>[];

  if (!rows.length) {
    const yearsRes = await pwFetch(
      `${m.url}/rest/v1/open_payments_company_totals?select=program_year&order=program_year.desc`,
      { headers: { apikey: m.key, Authorization: `Bearer ${m.key}` } },
    ).catch(() => null);
    const years = yearsRes?.ok ? [...new Set(((await yearsRes.json()) as { program_year: number }[]).map((r) => r.program_year))] : [];
    return {
      found: false,
      reason: company ? 'company_not_found' : 'year_not_loaded',
      program_year: year,
      years_available: years,
      hint: company
        ? `No manufacturer matching "${company}" reported payments in ${year}. Names are as the company filed them ("Lilly USA, LLC", not "Eli Lilly"), so try a distinctive fragment. A company absent here did not report payments that year — it is not a gap in the summary.`
        : `Program year ${year} is not loaded. CMS publishes a program year months after it ends, so a recent year may not exist yet rather than being empty.`,
    };
  }
  ...
  return {
    ...
    companies: rows.map((r) => ({
      ...
      distinct_recipients: Number(r.distinct_recipients),
    })),
    summary_refreshed_at: rows[0]?.refreshed_at ?? null,
    source: 'CMS Open Payments General Payment data, aggregated by Pipeworx',
    source_url: rows[0]?.source_url ?? null,
    interpretation: 'Totals are disclosed transfers of value from manufacturers and GPOs to physicians and teaching hospitals. They are a transparency record, not evidence of wrongdoing, and they exclude research and ownership payments, which CMS publishes separately.',
  };
}

async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'open_payments_top_companies':
      return topCompanies(args);
...
async function natureBreakdown(args: Record<string, unknown>) {
  ...
  if (filters.length !== 1) throw new Error('user_error: Provide exactly one of npi, company, or product.');
...
async function searchPayments(args: Record<string, unknown>) {
  ...
  if (!conditions.length) throw new Error('user_error: Provide at least one search filter.');
  ...
  const response = await pwFetch(`${BASE}/datastore/query`, {
...
async function resolveDataset(year: number, type: PaymentType) {
  ...
  const payload = await boundedJson(await pwFetch(url, { headers: { Accept: 'application/json' } }), 'CMS metadata search');
